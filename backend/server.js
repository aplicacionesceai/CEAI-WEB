...
const createTables = async () => {
  try {
    ...
    // ✅ Por si la tabla existía (migración segura)
    await pool.query(`ALTER TABLE proyectos ADD COLUMN IF NOT EXISTS lider TEXT`);
    await pool.query(`ALTER TABLE proyectos ADD COLUMN IF NOT EXISTS apoyo_tecnico TEXT`);
    await pool.query(`ALTER TABLE proyectos ADD COLUMN IF NOT EXISTS resumen TEXT`);
...
app.post('/api/proyectos', async (req, res) => {
  const { semillero_id, titulo, estado, lider, apoyo_tecnico, objetivo, resumen, imagenes } = req.body;

  if (!semillero_id || !titulo || !estado) {
    return res.status(400).json({ error: 'semillero_id, titulo y estado son obligatorios' });
  }

  try {
    const result = await pool.query(
      `INSERT INTO proyectos (semillero_id, titulo, estado, lider, apoyo_tecnico, objetivo, resumen, imagenes)
...
app.put('/api/proyectos/:id', async (req, res) => {
  ...
  try {
    await pool.query(
      `UPDATE proyectos
       SET semillero_id=$1, titulo=$2, estado=$3, lider=$4, apoyo_tecnico=$5, objetivo=$6, resumen=$7, imagenes=$8
       WHERE id=$9`,
      [
        semillero_id,
        titulo,
